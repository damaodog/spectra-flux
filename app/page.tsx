...
export default function Home() {
  const { state, hydrated, warning, remove, exportJson } = useCuration();
  const [playing, setPlaying] = useState(true);
  const [pageIndex, setPageIndex] = useState(0);
  const [notice, setNotice] = useState("策展首页已载入");
  ...
  const visibleEntries = state.showcase.slice(
    pageStart,
    pageStart + MOTION_PAGE_SIZE,
  );
  ...
  const removeFromHome = (entryId: string) => {
    remove(entryId);
        ...
        <aside className="control-rail">
          <div className="control-rail-inner">
            ...
            <section className="intro">
              ...
              <h1>策展首页</h1>
              <p>
                这里只展示你从效果库或随机实验室明确加入的动态作品。新作品始终排在最前。
              </p>
            </section>
            ...
            <footer>
              <span>LOCAL-FIRST CURATION · 12 WORKS PER PAGE</span>
